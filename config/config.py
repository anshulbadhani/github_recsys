...
class PathConfig:
    ...
    data_dir: Path = field(init=False) # init=False means do not handle in the constructor itself
    ...
    def __post_init__(self):
        ...
        self.outputs_dir = self.project_root / "outputs" # TODO: Setup outputs dir later
        # for logs of csv (but I am not planning to use CSVs)
    ...
    def get_repo_embeddings_path(self, backend: str = "cpu") -> Path:
        """Get path to repo embeddings."""
        return self.embeddings_dir / f"repo_embeddings_{backend}.pkl"
    
    def get_user_embeddings_path(self, backend: str = "cpu") -> Path:
        """Get path to user embeddings."""
        return self.embeddings_dir / f"user_embeddings_{backend}.pkl"
